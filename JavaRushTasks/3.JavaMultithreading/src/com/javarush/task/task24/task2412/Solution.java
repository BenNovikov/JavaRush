    ...
    public static void sort(List<Stock> list) {
        Collections.sort(list, new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                ...
                int result = 0;
                double result2 = 0;
                    result = stock1.get("name").toString().toUpperCase()
                            .compareTo(stock2.get("name").toString().toUpperCase());

                if (result != 0) return result;
                if (stock1.containsKey("date") && stock2.containsKey("date")) {
                try {
                    result = sdf.parse(stock2.get("date").toString())
                            .compareTo(sdf.parse(stock1.get("date").toString()));
                }
                catch (ParseException e) {}
                }
                if (result != 0) return  result;
                if (stock1.containsKey("open") && stock2.containsKey("open")) {
                    result2 = Double.valueOf((double) stock2.get("last") - (double) stock2.get("open"))
                            .compareTo(Double.valueOf((double) stock1.get("last") - (double) stock1.get("open")));
                }
                return (int)result2;
            }
        });
